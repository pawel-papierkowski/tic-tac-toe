...
describe('Tests of MiniMax algorithm.', () => {
  describe('Terminal results.', () => {
    it('result for board in win state', () => {
    ...
    it('result for board in loss state', () => {
      const board = createEmptyBoard();
      board[0][0] = EnCellState.X;
      board[1][1] = EnCellState.X;
      board[2][2] = EnCellState.X;
    ...
    it('result for board in draw state (no move possible)', () => {
  ...
  describe('Scoring results for empty board.', () => {
    it('result for empty board and 0 depth (scoring for this state of board only)', () => {
      const board = createEmptyBoard();
      const expectedResult = { score: 0, depth: 0, moves: [] };
      verifyMiniMaxForBoard(board, EnCellState.X, 0, expectedResult);
    });

    it('result for empty board and 1 depth (scoring for next move)', () => {
      const board = createEmptyBoard();
      // Will always try to get center cell as first move.
      const expectedResult = { score: 4, depth: 1, moves: [{ x: 1, y: 1 }] };
      verifyMiniMaxForBoard(board, EnCellState.X, 1, expectedResult);
    });

    it('result for empty board and 2 depth (scoring for two moves: you and opponent)', () => {
      const board = createEmptyBoard();
      // Will always try to get center cell as first move.
    ...
    it('result for empty board and 3 depth (scoring for three moves: you, opponent, you)', () => {
      const board = createEmptyBoard();
      // Will always try to get center cell as first move.
      const expectedResult = {
        score: 12,
  ...
  describe('Scoring results for mostly empty board.', () => {
    it('result for board with win for X in one move and 0 depth (scoring for this state of board only)', () => {
      const board = createEmptyBoard();
    ...
    it('result for board with win for X in one move and 1 depth', () => {
      const board = createEmptyBoard();
      board[0][1] = EnCellState.X;
      board[1][1] = EnCellState.X;
      // Current state of board:
    ...
    it('result for board with win for X in one move and 15 depth (but we find win immediately)', () => {
      const board = createEmptyBoard();
      board[0][1] = EnCellState.X;
      board[1][1] = EnCellState.X;
      // Current state of board:
    ...
    it('result for board with multiple win moves for X', () => {
      const board = createEmptyBoard();
      board[0][1] = EnCellState.X;
      ...
      // X has three moves where it can win: 0,0 and 2,0 and 2, 1.
      // Due to details of minMax algo implementation, it will always pick winning move closest to right bottom
      // of board if multiple winning moves are available.
      const expectedResult = { score: 1000, depth: 1, moves: [{ x: 2, y: 1 }] };
      verifyMiniMaxForBoard(board, EnCellState.X, 3, expectedResult);
    });

    it('result when prioritizing own win over blocking opponent win', () => {
      const board = createEmptyBoard();
      board[0][0] = EnCellState.X;
      board[0][1] = EnCellState.X;
      board[2][1] = EnCellState.O;
      ...
      // X can win at 0,2 or block opponent at 2,0. X should prioritize win over blocking.
      const expectedResult = {
        score: 1000,
        depth: 1,
        moves: [{ x: 0, y: 2 }],
  ...
  describe('Scoring results for realistic board states.', () => {
    it('result for board where most O moves are good', () => {
      const board = createEmptyBoard();
      board[0][1] = EnCellState.X;
    ...
    it('result for board where O is about to lose (minimal)', () => {
      const board = createEmptyBoard();
      board[1][1] = EnCellState.X;
    ...
    it('result for board where O is about to lose (further)', () => {
      const board = createEmptyBoard();
      board[0][2] = EnCellState.X;
      board[1][1] = EnCellState.O;
      board[2][0] = EnCellState.O;
  ...
  describe('Other results.', () => {
    it('scores same board differently depending on whose perspective (X vs O)', () => {
      const board = createEmptyBoard();
      board[1][1] = EnCellState.X;

      const expectedResultX = {
        score: 22,
        depth: 3,
        moves: [
          { x: 2, y: 2 },
      ...
      verifyMiniMaxForBoard(board, EnCellState.X, 3, expectedResultX);
      ...
      verifyMiniMaxForBoard(board, EnCellState.O, 3, expectedResultY);
    });
  });
});
