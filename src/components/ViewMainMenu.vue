...
function resolveDiffLabelClass() {
  if (gameState.value.settings.whoFirst === EnWhoFirst.HumanVsHuman)
    return 'difficulty-label-disabled';
  return '';
}

function resolveDiffSelectClass() {
  if (gameState.value.settings.whoFirst === EnWhoFirst.HumanVsHuman)
    return 'difficulty-select-disabled';
  return '';
}
</script>
...
<template>
  <div class="menu">
    ...
    <select
      id="whoFirst"
      data-testid="select-whoFirst"
      v-model.number="gameState.settings.whoFirst"
    >
      <option v-for="(desc, value) in whoFirstDescr" :key="value" :value="Number(value)">
        {{ desc }}
      </option>
    </select>
    <label :class="resolveDiffLabelClass()" for="difficulty"> AI Difficulty: </label>
    <select
      :class="resolveDiffSelectClass()"
      :disabled="gameState.settings.whoFirst === EnWhoFirst.HumanVsHuman"
      id="difficulty"
      data-testid="select-difficulty"
      v-model.number="gameState.settings.difficulty"
    >
